   ...
   TokenService.checkPermission(["EMD2"]),
    async (req, res, next) => {
        ...
        if (existingRecord.data) {
            return res
                .status(409)
                .json({ error: "Name,Code  Already Exists." });
        }
   ...
   // TokenService.checkPermission(["EMD4"]),
    async (req, res) => {
        try {
            ...
            const hasAssignedCourses = await courseModel.exists({
                groupId,
                departmentId,
            });
    ...
    try {
        ...
        if (!Array.isArray(departmentId) || departmentId.length === 0) {
            return res.status(400).json({
                success: false,
                message: "Invalid or empty departmentId array",
            });
        }

        const numericIds = departmentId.map((id) => {
            const num = parseFloat(id);
            if (isNaN(num)) {
                throw new Error(`Invalid numeric ID: ${departmentId}`);
            }
            return num;
        });
