...
class TokenService {
    static isAdmin(decodedToken) {
        return (
            decodedToken.role &&
            decodedToken.role.name.toLowerCase().includes("admin")
        );
    }

    static getIndividualPermissions(decodedToken) {
        const permissionsArray = decodedToken.role.permissions.reduce(
            (acc, permission) => {
        ...
        return async (req, res, next) => {
            if (!TokenService.isAuthenticated(req)) {
                return res
                    ...
                    .json({
                        message:
                            "Invalid token, you do not have access to call this",
                    });
            }
            try {
