...
class ReminderService extends BaseService {
    ...
    async updateDataById(reminderId, groupId, newData) {
        try {
            const updatedData = await ReminderModel.findOneAndUpdate({ reminderId: reminderId, groupId: groupId }, newData, { new: true });
            return updatedData;
        } catch (error) {
            throw error;
        }
    ...
    async deleteByDataId(reminderId, groupId) {
        try {
            const deleteData = await ReminderModel.deleteOne({ reminderId: reminderId, groupId: groupId });
            return deleteData;
        } catch (error) {
            throw error;
        }
