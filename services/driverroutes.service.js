...
class DriverRoutesService extends BaseService {
    ...
    async getByBusRouteId(groupId, driverRouteId) {
        console.log("query",{ groupId: groupId, driverRouteId: driverRouteId });
        let driverRoutes = await DriverRoutesModel.findOne({ groupId: Number(groupId), driverRouteId: Number(driverRouteId) })
        ...
        let route = await BusRoutesModel.findOne({ groupId: groupId, routeId: routeId });

        const driverId = route.driverId
        if (!driverId) {
            throw new Error("driverId not found.");
        }
        let driver = await DriverModel.findOne({ groupId: groupId, driverId: driverId });
